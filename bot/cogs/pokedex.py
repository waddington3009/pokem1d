...
class Pokedex(commands.Cog, name="Coleção"):
    ...
    # ------------------------------------------------------------------
    @commands.command(name="release", aliases=["soltar"])
    @commands.guild_only()
    async def release(self, ctx: commands.Context, numero: int) -> None:
        """Solta um pokémon (recebe algumas moedas). Favoritos são protegidos."""
        async with session_scope() as session:
            user = await helpers.fetch_user(session, ctx.author.id)
            poke = await helpers.get_pokemon_by_idx(session, user.id, numero)
            if poke is None:
            ...
            if poke.favorite:
                await ctx.send(embed=embeds.err_embed(
                    "Esse pokémon é favorito! Use `unfavorite` antes de soltar."
                ))
                return
            sp = POKEDEX.get(poke.species_id)
        ...
        await ctx.send(embed=embeds.ok_embed(
            "Pokémon solto",
            f"👋 Você soltou **{sp.name}** #{numero} e recebeu **{reward}** PokéCoins."
        ))
